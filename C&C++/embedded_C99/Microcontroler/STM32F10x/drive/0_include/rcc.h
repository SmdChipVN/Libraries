...
typedef struct
{
  ...
  BUNION(CIR, unsigned int,                     //              0               Clock interrupt register
    ...
    PLL2_RDY_IE                         , 1,    //13            0               PLL2 ready interrupt enable
    PLL3_RDY_IE                         , 1,    //14            0               PLL3 ready interrupt enable
    _reserved                           , 1,
    ...
    PLL2_RDYC                           , 1,    //21            0               PLL2 ready interrupt clear
    PLL3_RDYC                           , 1,    //22            0               PLL3 ready interrupt clear
  ...
  BUNION(APB1_RSTR, unsigned int,               //                              APB1 peripheral reset register
    ...
    _reserved4                          , 1,
    BKP                                 , 1,    //27            0               Backup interface reset
    PWR                                 , 1,    //28            0               Power interface reset
    DAC                                 , 1,    //29            0               DAC interface reset
    _reserved5                          , 2);
  ...
  BUNION(APB2_ENR, unsigned int,                //                              APB2 peripheral clock enable register
    ...
    ADC1                                , 1,    //9             0               ADC 1 interface clock enable
    ADC2                                , 1,    //10            0               ADC 2 interface clock enable
    ...
    ADC3                                , 1,    //15            0               ADC3 interface clock enable
    _reserved1                          , 3,
  ...
  BUNION(APB1_ENR, unsigned int,                //                              APB1 peripheral clock enable register
    ...
    _reserved4                          , 1,
    BKP                                 , 1,    //27            0               Backup interface clock enable
    PWR                                 , 1,    //28            0               Power interface clock enable
    DAC                                 , 1,    //29            0               DAC interface clock enable
    _reserved5                          , 2);
  ...
  BUNION(AHB_STR, unsigned int,                 //                              AHB peripheral clock reset register
    _reserved                           , 12,
...
typedef struct
{
  RSTRUCT(CR, unsigned int,                     //                              Clock control register
    HSION                                  ,    //0             1               Internal high-speed clock enable
    const HSI_RDY                          ,    //1             1               Internal high-speed clock ready flag
    _reserved                              ,
    HSI_TRIM                            [5],    //3:7           16              Internal high-speed clock trimming
    const HSI_CAL                       [8],    //8:15          -               Internal high-speed clock calibration
    ...
    _reserved2                          [2]);
  RSTRUCT(CFGR, unsigned int,                   //                              Clock configuration register
    SW                                  [2],    //0-1           0               System clock switch
    const SWS                           [2],    //2-3           0               System clock switch status
    AHB_PRE                             [4],    //4:7           0               AHB prescaler
    APB1_PRE                            [3],    //8:10          0               APB low-speed prescaler (APB1)
    ...
    MCO                                 [4],    //24:27         0               Microcontroller clock output
    _reserved1                          [4]);
  RSTRUCT(CIR, unsigned int,                    //              0               Clock interrupt register
    const LSI_RDYF                         ,    //0             0               LSI ready interrupt flag
    const LSE_RDYF                         ,    //1             0               LSE ready interrupt flag
    const HSI_RDYF                         ,    //2             0               HSI ready interrupt flag
    const HSE_RDYF                         ,    //3             0               HSE ready interrupt flag
    const PLL_RDYF                         ,    //4             0               PLL ready interrupt flag
    const PLL2_RDYF                        ,    //5             0
    const PLL3_RDYF                        ,    //6             0
    const CSSF                             ,    //7             0               Clock security system interrupt flag
    LSI_RDY_IE                             ,    //8             0               LSI ready interrupt enable
    LSE_RDY_IE                             ,    //9             0               LSE ready interrupt enable
    HSI_RDY_IE                             ,    //10            0               HSI ready interrupt enable
    HSE_RDY_IE                             ,    //11            0               HSE ready interrupt enable
    PLL_RDY_IE                             ,    //12            0               PLL ready interrupt enable
    PLL2_RDY_IE                            ,    //13            0               PLL2 ready interrupt enable
    PLL3_RDY_IE                            ,    //14            0               PLL3 ready interrupt enable
    _reserved                              ,
    LSI_RDYC                               ,    //16            0               LSI ready interrupt clear
    LSE_RDYC                               ,    //17            0               LSE ready interrupt clear
    HSI_RDYC                               ,    //18            0               HSI ready interrupt clear
    HSE_RDYC                               ,    //19            0               HSE ready interrupt clear
    PLL_RDYC                               ,    //20            0               PLL ready interrupt clear
    PLL2_RDYC                              ,    //21            0               PLL2 ready interrupt clear
    PLL3_RDYC                              ,    //22            0               PLL3 ready interrupt clear
    CSSC                                   ,    //23            0               Clock security system interrupt clear
    _reserved1                          [8]);
  RSTRUCT(APB2_RSTR, unsigned int,              //                              APB2 peripheral reset register
  ...
  RSTRUCT(APB1_RSTR, unsigned int,              //                              APB1 peripheral reset register
    ...
    CAN                                    ,    //25            0               CAN reset
    _reserved4                             ,
    BKP                                    ,    //27            0               Backup interface reset
    PWR                                    ,    //28            0               Power interface reset
    DAC                                    ,    //29            0               DAC interface reset
    _reserved5                          [2]);
  RSTRUCT(AHB_ENR, unsigned int,                //                              AHB peripheral clock enable register
    DMA1                                   ,    //0             0               DMA1 clock enable
    DMA2                                   ,    //1             0               DMA2 clock enable
    SRAM                                   ,    //2             1               SRAM interface clock enable
    _reserved                              ,
    ...
    _reserved4                          [21]);
  RSTRUCT(APB2_ENR, unsigned int,               //                              APB2 peripheral clock enable register
    ...
    ADC1                                   ,    //9             0               ADC 1 interface clock enable
    ADC2                                   ,    //10            0               ADC 2 interface clock enable
    ...
    ADC3                                   ,    //15            0               ADC3 interface clock enable
    _reserved1                          [3],
    TIM9                                   ,    //19            0               TIM9 timer clock enable
    TIM10                                  ,    //20            0               TIM10 timer clock enable
    TIM11                                  ,    //21            0               TIM11 timer clock enable
    _reserved2                          [10]);
  RSTRUCT(APB1_ENR, unsigned int,               //                              APB1 peripheral clock enable register
    ...
    _reserved4                             ,
    BKP                                    ,    //27            0               Backup interface clock enable
    PWR                                    ,    //28            0               Power interface clock enable
    DAC                                    ,    //29            0               DAC interface clock enable
    _reserved5                          [2]);
  RSTRUCT(BDCR, unsigned int,                   //                              Backup domain control register
    ...
    _reserved2                          [15]);
  RSTRUCT(CSR, unsigned int,                    //                              Control/status register
    LSI_ON                                 ,    //0             0               Internal low-speed oscillator enable
    const LSI_RDY                          ,    //1             0               Internal low-speed oscillator ready
    _reserved                           [22],
  ...
  RSTRUCT(AHB_STR, unsigned int,                //                              AHB peripheral clock reset register
    _reserved                           [12],
    OTGFS_RST                              ,    //12            0               USB OTG FS reset
    _reserved1                             ,
    ETHMAC_RST                             ,    //14            0               Ethernet MAC reset
    _reserved2                          [17]);
  RSTRUCT(CFGR2, unsigned int,                  //                              Clock configuration register 2
    PRE_DIV1                            [4],    //0:3           0               PREDIV1 division factor
    PRE_DIV2                            [4],    //4:7           0               PREDIV2 division factor
    PLL2_MUL                            [4],    //8:11          0               PLL2 Multiplication Factor
    PLL3_MUL                            [4],    //12:15         0               PLL3 Multiplication Factor
